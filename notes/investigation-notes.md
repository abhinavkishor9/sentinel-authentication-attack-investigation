...
Authentication attack investigation using Microsoft Sentinel and KQL.
...
- Source IP changes for the same account
...
A real SOC investigation would correlate this authentication activity with:
...
- Endpoint telemetry
- Threat intelligence
- Activity after the successful authentication

## Key Investigation Logic

The main detection pattern demonstrated by the lab is:

`One source IP → multiple accounts → repeated failures → successful authentication`

This pattern should trigger further investigation rather than an automatic conclusion of compromise.
